...
const daily_work_details = {
	...
	'store-overall-report-endpoint':"monthly-report/report-update", //update details button
		...
		if($("#page_type").val() == daily_work_details['page-value'] || $("#page_type").val() == admin_work_details['page-value'] || $("#page_type").val() == admin_approval_details['page-value']){
			...
			*/
			...
			if($("#page_type").val() == admin_approval_details['page-value']){
				// Add status code for sending data to Overall Report Endpoint start.
				var send_date = new Date();
				var s_date = send_date.getDate();
				request_json["date_"+s_date] = status_code[status];
				// Add status code for sending data to Overall Report Endpoint end.
			}
			if($("#page_type").val() != admin_work_details['page-value']){
				$.ajax({
				headers: { 
			...
			if($("#page_type").val() == admin_approval_details['page-value'] || $("#page_type").val() == admin_work_details['page-value']){
				// 23-01-2022 Bala - Start
				...
				console.log(post_endpoint);
				// 23-01-2022 Bala - End
				var customer_id = parseInt($("."+row).find('.customer_id').text()) ? $("."+row).find('.customer_id').text() : 0;
				$.ajax({
					headers: { 
						'Accept': 'application/json',
						'Content-Type': 'application/json' 
					...
					method: "POST",
					// Send customer id in query parameter 23-01-2022 Start Bala
					url: base_url+post_endpoint+"?customer_id="+""+customer_id,
					// Send customer id in query parameter 23-01-2022 Start end
					}).done(function( data) {
						alert(data_updated_message);
					});
			}
