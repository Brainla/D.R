...
const admin_approval_details = {
	...
	'search-endpoint':"admin-approval/single-custromer-details",
	// Add insert Endpoint in call for Monthly report from Admin Approval 22-01-2022 start Bala
	'store-approval-endpoint':"monthly-report/insert",
	// Add insert Endpoint in call for Monthly report from Admin Approval 22-01-2022 end Bala

	// Add Removal Endpoint in delete call for Admin Approval 22-01-2022 start Bala
	'removal-endpoint':'admin-approval/delete',
	// Add Removal Endpoint in delete call for Admin Approval 22-01-2022 start Bala
	'export-endpoint':"daily-work-export",
	'data-accessors':{'id':'customer_id','name':'customer_name','emi':'emi','pending':'previous_balance','current_balance':'current_balance','status':'status','collection_date':'collection_date','rec_amt':'received_amount','agent_id':'agent_id','given_date':'given_date'}
};
...
$(document).ready(function(){
		...
		$('#updateUser').on('show.bs.modal', function(e) {
			var updateUserId = $('#updateButton').data('user-id');
	...
	$(document).on('click','.saveButton, .removeButton',function(e){
		...
		if($("#page_type").val() == daily_work_details['page-value'] || $("#page_type").val() == admin_work_details['page-value'] || $("#page_type").val() == admin_approval_details['page-value']){
			...
			// Send only customer ID in delete call for Admin Work 22-01-2022 start Bala
			if($("#page_type").val() == admin_work_details['page-value']){
				request_json = { customer_id:parseInt($("."+row).find('.customer_id').text()) ? parseInt($("."+row).find('.customer_id').text()) : 0 }
			}
			// Send only customer ID in delete call for Admin Work 22-01-2022 end Bala
			$.ajax({
			headers: { 
				'Accept': 'application/json',
				'Content-Type': 'application/json' 
			...
			// Send delete call for Admin Approval 22-01-2022 start Bala
			if($("#page_type").val() == admin_approval_details['page-value']){
				post_endpoint = admin_approval_details['removal-endpoint'];;
				request_json = { customer_id:parseInt($("."+row).find('.customer_id').text()) ? parseInt($("."+row).find('.customer_id').text()) : 0 }
				$.ajax({
					headers: { 
						'Accept': 'application/json',
			...
			// Send delete call for Admin Approval 22-01-2022 end Bala
			
			// Added if condition for approval page start Bala
		if($("#page_type").val() == admin_work_details['page-value'] || $("#page_type").val() == admin_approval_details['page-value']){
			// Added if condition for approval page end Bala
			$("."+row).remove();
	...
	$(document).on('click','.add-customer',function(e){
		...
		var request_json = {
			...
			agent_id: $('#agent').val() ? $('#agent').val() : 0,
			...
			collection_date: $('#collectionDate').val() ? $('#collectionDate').val() : 0,
	...
	$(document).on('click','.update-agent-btn',function(e){
		...
		var request_json = {
			agent_id: $('#updateUserId').val() ? $('#updateUserId').val() : 0,
			ph_no: $('#updateMobile').val() ? $('#updateMobile').val() : 0
		}
		$.ajax({
			headers: { 
				'Accept': 'application/json',
			...
			method: "POST",
			data:JSON.stringify(request_json),
			url: base_url+post_endpoint+"?agent_id="+$('#updateUserId').val()
			}).done(function( data) {
				alert(data_updated_message);
			});
	});
		...
		if(total_count > 0 || checkValid(search_val)){
			...
			if(data['dataList']){
				$.each(dataList, function(index,val){
					...
					'<td class="customer_name">'+(val[monthly_report_details['data-accessors']['agent_id']]!=undefined ? val[monthly_report_details['data-accessors']['agent_id']] : "")+'</td>'+
					...
					'<td class="customer_name">'+(val[monthly_report_details['data-accessors']['current_balance']]!=undefined ? val[monthly_report_details['data-accessors']['current_balance']] : "")+'</td>'+
					...
					'<td class="status">'+(val[monthly_report_details['data-accessors']['balance']]!=undefined ? val[monthly_report_details['data-accessors']['balance']] : "")+'</td>'+
...
function deleteAgent(deleteId) {
	...
	const deleteURL = base_url + create_agent_details['delete-agent'] + "?agent_id="  + deleteId
	$.ajax({
		method: "POST",
		url: deleteURL
		}).done(function( data) {
...
function changeDateFormat(date){
	...
	// 22-10-2022 Changed for setting date when digits came in single number.  - Bala
	if(curr_month < 10){
